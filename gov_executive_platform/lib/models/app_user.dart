...
class AppUser {
  ...
  /// إداراتُ هذا الحساب — **القراءةُ الواحدة**: القائمةُ إن وُجدت، وإلا
  /// المفردُ الموروث، وإلا فارغة.
  ///
  /// ــــ ولماذا هنا لا في كل موضعٍ يحتاجها ــــ
  ///
  /// الإدارةُ على الحساب في حقلين: [departmentId] مفرداً (الأقدم، يحمله كلُّ
  /// حساب) و[departmentIds] قائمةً (أُضيفت حين صار مديرُ الإدارة قد يدير
  /// أكثر من إدارة). وقد قُرئا **قراءاتٍ مختلفة في المنصة الواحدة**، فكان
  /// مديرُ إدارةٍ بطاقتُه تحمل المفرد وحده يرى قلمَ التعديل ويُردّ عليه
  /// الحفظ. وكلّف ذلك جولةً كاملة — راجع `test_rules/dept_claim.rules.test.mjs`.
  ///
  /// فصارت القراءةُ هنا وحدها، ويُحوَّل إليها كلُّ من يحتاجها.
  List<String> get allDepartmentIds {
    if (departmentIds.isNotEmpty) return departmentIds;
    final one = departmentId;
    return (one == null || one.isEmpty) ? const [] : [one];
  }

  /// هل ينتمي هذا الحساب إلى هذه الإدارة؟
  ///
  /// و`null` أو فراغٌ يُقرأ «لا إدارةَ محدّدة» فيُعاد `false`: مسؤولُ النظام
  /// والمستخدم التنفيذي بلا إدارة، ولا يُقال إنهما في كل إدارة من هنا —
  /// نطاقُهما يُقرَّر بأدوارهما لا بانتمائهما.
  bool belongsToDepartment(String? id) =>
      id != null && id.isNotEmpty && allDepartmentIds.contains(id);

  /// هل يُراسَل هذا الحساب؟ — تعريفٌ واحد، تقرؤه كلُّ قائمة مستلمين.
  ///
  /// ــــ لماذا لا يكفي [active] ــــ
  ///
  /// بعد أن صار التوقيف يحذف حساب الدخول، صارت العودة تمرّ بتسجيلٍ جديد
  /// يُدمَج مع القديم. فيبقى في `users` **مستندان بالاسم والبريد نفسيهما**:
  /// القديمُ الموقوف وعليه [mergedIntoUid]، والجديد المعتمَد. وقوائم
  /// المستلمين تُطابق بالاسم في مواضع، فتُرجع التوأمين معاً — فيصل الشخصَ
  /// الواحد بريدان، ويرى مسؤول النظام صفّين متطابقين لا يفرّق بينهما.
  ///
  /// والشرط الثاني هو ما يحسم ذلك: الموقوف المندمج ليس شخصاً آخر يُراسَل،
  /// بل ظلُّ حسابٍ انتقل صاحبُه. أما الموقوف غير المندمج فيسقط بـ[active]
  /// وحدها كما كان.
  bool get isContactable => active && mergedIntoUid == null;
