...
enum RolePermission {
  ...
  // ــ اعتمادُ التغييرات: بوّابةُ لجنة CAB ــ
  //
  // **ورفعُ التغيير ليس منها**: مكتبُ الخدمة يرفع بـ`htk`، وهذه للبتّ.
  // فلا يعتمد أحدٌ تغييرَ نفسِه — والخادمُ يفرضه، لا الشاشة.
  //
  // ــ ولماذا لا تُركَب على `agd` ــ
  //
  // «اعتماد القرارات العامة» للقرارات التنفيذيّة. والرُّكوبُ يمنح حامليها
  // **اليومَ** اعتمادَ تغييرات البنية التقنيّة — إطفاءَ خادمٍ، تبديلَ
  // إعداداتِ شبكة — بلا أن يطلبه أحد. وذلك بعينه ما يُنهى عنه: صلاحيةٌ
  // تتّسع بأثرٍ جانبيٍّ لتغييرٍ في مكانٍ آخر.
  //
  // ومغلقةٌ لكلّ دورٍ افتراضاً، تُمنح بالاسم وتُسحب — كـ`vpc` و`epc`
  // و`htk`.
  approveChanges('cab', 'اعتماد التغييرات التقنية',
      'البتّ في طلبات التغيير ومراجعةُ ما نُفِّذ طارئاً'),

  // ــ سجلُّ الأنظمة والأصول ــ
  //
  // **ولا يُركَب على `htk`**: مكتبُ الخدمة يعالج البلاغات، وأمينُ الأصول
  // يمسك السجلّ — وقد يكونان شخصين. والرُّكوبُ يمنح كلَّ فنّيّ دعمٍ
  // **اليومَ** قراءةَ قيم العقود ومعلوماتِ المورّدين بلا أن يطلبه أحد.
  //
  // ومغلقةٌ لكلّ دورٍ افتراضاً، لا تُورَّث لدورٍ مخصّص، تُمنح بالاسم
  // وتُسحب — كأخواتها.
  manageItAssets('ita', 'سجلّ الأنظمة والأصول',
      'الأنظمةُ والأجهزةُ والمورّدون والعقود: الاطّلاعُ والتحرير');
