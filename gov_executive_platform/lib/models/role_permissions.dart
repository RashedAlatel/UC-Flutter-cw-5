...
enum RolePermission {
  ...
  deleteRecords('del', 'حذف السجلات',
      'حذف المشاريع والأعمال حذفاً منطقياً داخل إداراته — يستعيده مسؤول النظام'),
  sendNotifications('ntf', 'إرسال الإشعارات', 'مراسلة المستخدمين بالبريد وواتساب ضمن نطاقه'),
  // ــ تنبيه المتأخرات جماعياً: صلاحية مستقلّة عن `ntf` بقصد ــ
  //
  // `ntf` تفتح **الرسالة الحرّة** إلى أي مستخدم من شاشة المستخدمين، وهي
  // أوسع بكثير مما يحتاجه من يتابع مشاريع إدارته. ودمجُهما كان يعني أن من
  // أردنا له تنبيه المتأخرين نالَ معه مراسلة الوزارة كلها بنصٍّ يكتبه.
  //
  // وهي **ليست بوابة إرسال**: البريد لا يخرج بها. من ليس مسؤول نظام يبقى
  // طلبُه مرفوعاً إلى `notifySend` كسائر البريد — وهذه تقرّر من يحقّ له أن
  // **يطلب** لا من ينفّذ.
  bulkDelayAlerts('bla', 'تنبيه المشاريع المتأخرة جماعياً',
      'تحديد عدة مشاريع متأخرة وإرسال تنبيه تأخير لمسؤوليها دفعةً واحدة'),
  ...
  // ــ تعديلُ مواعيد المهامّ: صلاحيةٌ مستقلّة بحدٍّ مكتوب ــ
  //
  // وهي **ليست بوابة المواعيد النهائية**، ولا تقترب منها: تلك موعدُ
  // **المشروع**، محصورةٌ بمسؤول النظام ولا يفتحها مفتاحٌ مفوَّض أبداً. وهذه
  // موعدُ مهمةٍ داخل مشروعٍ قائم، **ولا يتجاوز موعدَ مشروعها** — والحدُّ
  // مفروضٌ في `taskDueDateRejection` وفي قاعدة `tasks` معاً، لا في الشاشة.
  //
  // فمن نالها لا يستطيع بها أن يمدّ التزامَ الوزارة يوماً واحداً؛ يعيد
  // ترتيب العمل داخل مدّةٍ أقرّها مسؤولُ النظام.
  manageTaskDates('mtd', 'تعديل مواعيد المهام',
      'إعادة جدولة مهامّ مشاريع إدارته ضمن موعد المشروع — لا يتجاوزه'),
  ...
  // ــــ دليلُ الإجراءات: صلاحيتان، القراءةُ والتحرير ــــ
  //
  // والقراءةُ صلاحيةٌ مستقلّة بقرارٍ صريح: الدليلُ يصف كيف تسير أعمالُ
  // الوزارة، فليس كلُّ ما فيه لكل من دخل المنصة. ومغلقتان في `defaults()`
  // فلا يظهر المدخلُ لأحد قبل قرار مسؤول النظام.
  //
  // **والمفتاحان `vpc` و`epc` لا `mpc`**: `mpc` تجاور `mpr` (إنشاء
  // المشاريع) حرفاً واحداً، وبوّابتان متجاورتان في الاسم أوّلُ ما يُوقع في
  // الخطأ. وقد وقع في هذه المنصة بين `pendingTransferFor` ونظيرتها.
  viewProcedures('vpc', 'الاطّلاع على دليل الإجراءات',
      'قراءة الإجراءات وخطواتها ومرفقاتها ونسخها السابقة'),

  // ومن يحرّر يقرأ بالضرورة — يقرّره `AppStore.canViewProcedures` وقاعدةُ
  // `procedures` بالصيغة نفسِها، فلا يُمنح أحدٌ تحريراً بلا قراءة فيرى
  // شاشةً فارغة.
  editProcedures('epc', 'تحرير دليل الإجراءات',
      'إضافة الإجراءات وتعديل خطواتها وأرشفتها — ولا تُحذف، وكل نسخة تُحفظ'),

  // ــ الاطّلاع على حالات الموظفين اليومية: منحةٌ مُنطاقة ــ
  //
  // وهي **مُنطاقةٌ لا دوريّة** كـ`mpr` و`apr`: حالاتُ الحضور والإجازات
  // بياناتٌ شخصيّةٌ عن أشخاص بأعيانهم، لا أرقامُ مشاريع. ومن يملك متابعتَها
  // يملك أن يعرف متى استأذن فلانٌ وكم مرّةً مرض. فلا يُمنح ذلك لدورٍ يقع
  // على كل من يحمله، بل **لفردٍ بعينه في إدارةٍ مسمّاة، وبقرارٍ يُسحب**.
  //
  // ولا تُغني عنها الأدوار: مديرُ الإدارة ورئيسُ القسم يريان بحكم موقعهما
  // لا بهذا المفتاح — راجع `canViewStatuses()` في القواعد. وهذه للمراقب
  // الذي يتابع ولا يدير.
  viewDailyStatuses('vds', 'الاطّلاع على حالات الموظفين اليومية',
      'قراءة حالات الحضور والإجازات والمهامّ اليومية — ضمن نطاق محدَّد'),

  // ــ مركزُ قيادة القطاع: مدخلٌ لا بوابة ــ
  //
  // هذه الصلاحيةُ **ترتيبٌ لا حراسة**: تقرّر ظهورَ مدخل «مركز القيادة» في
  // القائمة، ولا تفتح بياناً لم يكن مفتوحاً. فالمركزُ يُحسب من
  // `visibleProjects` و`visibleWorks` و`trackablePeople` — وكلُّها مصفّاةٌ
  // بنطاق القارئ أصلاً، وقواعدُ Firestore لم تتغيّر.
  //
  // فمن فُتح له المركزُ لم يُمنح بياناً جديداً؛ رأى ما يراه أصلاً مجموعاً
  // بصورةٍ تُقرأ في دقيقة. وهو نمطُ `dsh` و`dpg` القائم.
  viewCommandCenter('vcc', 'عرض مركز قيادة القطاع',
      'ظهور مدخل مركز القيادة — ما يحتاج تدخّلاً وحملُ الفريق، ضمن نطاقه'),

  // ــ معالجةُ البلاغات: بوّابةُ مكتب الخدمة ــ
  //
  // **وفتحُ البلاغ ليس منها**: كلُّ موظّفٍ معتمدٍ يفتح بلاغَه ويقرؤه
  // بمِلكيّته إيّاه لا بمفتاح — كما يقرأ صاحبُ الشكوى شكواه. فهذه للطرف
  // الآخر: من يرى الطابورَ كلَّه ويُسنِد ويُغلق.
  //
  // ــ ولماذا مفتاحٌ جديدٌ ولم يُركَب على `vad` أو `mw` ــ
  //
  // لأنّ الرُّكوبَ يمنح حامليهما **اليومَ** نافذةً على بلاغات الوزارة
  // كلِّها بلا أن يطلبها أحد، ولا أن يقرّرها مسؤولُ النظام. وذلك بعينه ما
  // يُنهى عنه: صلاحيةٌ تتّسع بأثرٍ جانبيٍّ لتغييرٍ في مكانٍ آخر.
  //
  // ــ وهو واسعٌ بلا نطاقٍ إداريّ، وذلك مقصود ــ
  //
  // مكتبُ الخدمة يستقبل من الوزارة كلِّها، فقصرُه على إدارة حامله يجعله
  // لا يرى ما جاء إليه. فثمنُ السعة يُدفع من جهةٍ أخرى: **مغلقٌ لكلّ دورٍ
  // افتراضاً**، لا يُمنح إلا بالاسم، ويُسحب. وهو ما فُعل بـ`vpc` و`epc`.
  handleTickets('htk', 'معالجة البلاغات وطلبات الخدمة',
      'الاطّلاع على كل البلاغات وإسنادها وتغيير حالتها وإغلاقها');
  ...
  static const Set<RolePermission> scoped = {
    ...
    RolePermission.viewDailyStatuses,
  };
...
class RolePermissionsConfig {
  ...
  factory RolePermissionsConfig.defaults() => const RolePermissionsConfig({
        ...
        // و'bla' للأدوار الثلاثة التي تتابع مشاريع لا تنفّذها وحدها:
        // المستخدم التنفيذي ومدير الإدارة ومدير المشروع. و«موظف» خارجها
        // بقرار صريح — يمنحها له مسؤول النظام من الشبكة إن أراد.
        // و'vpc' و'epc' (دليل الإجراءات) **مغلقتان لكل دور** بقرارٍ صريح:
        // الدليلُ يُفتح بمنحٍ لا بحكم الدور، ولا يظهر مدخلُه لأحد قبل ذلك.
        // و`_knownKeys` يجعل الغائبَ عن المستند المخزَّن يُقرأ من هنا لا
        // منعاً — فبقاؤهما خارج هذه الخريطة هو ما يُبقي البابَ مغلقاً.
        // و'vcc' (مركز قيادة القطاع) مع 'dsh': مدخلٌ لا بوابة — يعرض ما
        // يراه القارئُ أصلاً مجموعاً، ولا يفتح بياناً جديداً. والجدولان
        // هنا وفي `DEFAULT_ROLE_PERMS` على الخادم يجب أن يبقيا سواءً —
        // وقد انحرفا من قبل في 'bla'.
