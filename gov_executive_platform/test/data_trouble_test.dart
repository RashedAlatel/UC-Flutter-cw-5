...
void main() {
  group('صنفُ العطل يُسمّى', () {
    ...
    // ــ والنصيحةُ تُقاس كما يُقاس التصنيف ــ
    //
    // ــــ العطل الذي أوجد هذا الاختبار ــــ
    //
    // كان هذا الفرعُ مقيساً في **تصنيفه** خمسَ مرّات، ولم يُقَس متنُه مرّة.
    // فشُحن وهو يُملي:
    //
    //     firebase deploy --only firestore:indexes
    //
    // فنفّذه صاحبُ الجهاز، فسقط بـ`403 ... project dafault`: أداةُ Firebase
    // تحفظ «المشروع النشط» خارج المستودع، والأمرُ المجرّد يلتقط تلك الحالة
    // لا `.firebaserc`. وقد سقط النشرُ بها مرّةً قبل هذه، ولذلك كُتب
    // `tool/deploy.sh` يقرأ المعرّف ويمرّره في كلّ أمر.
    //
    // **فصنّف الفرعُ العطلَ صواباً وأملى علاجاً يسقط.** ولافتةٌ تُصيب في
    // التسمية وتُخطئ في العلاج ليست أنفعَ من لافتةٍ صامتة: كلتاهما تُضيّع
    // اليوم.
    test('والعلاجُ المُملى هو أمرُ النشر المسنون لا الأمرَ المجرّد', () {
      final r = AppStore.describeDataErrors(
        {'dailyStatuses/قسمي': 'The query requires an index. See https://x/y'},
        const {},
      );
      expect(r.body, contains('./tool/deploy.sh'),
          reason: 'وهو الأمرُ الذي يمرّر معرّفَ المشروع صراحةً');
      expect(r.body, isNot(contains('firebase deploy')),
          reason: 'الأمرُ المجرّد يلتقط مشروعاً محفوظاً خارج المستودع فيسقط بـ403');
    });
