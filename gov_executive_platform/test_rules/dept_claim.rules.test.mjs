...
/**
 * ــــ لماذا تبدّلت مركبةُ القياس من الاسم إلى القسم ــــ
 *
 * كان هذا الملفّ يقيس البطاقة بكتابة **اسم المشروع**، وهو ما بلّغ عنه
 * مسؤول النظام يومها. ثم صار الاسمُ يمرّ بمسار الاعتماد بقرارٍ صريح منه
 * (راجع `test_rules/project_edit.rules.test.mjs`)، فما عاد أحدٌ — مديرَ
 * إدارةٍ كان أو مديرَ مشروع — يكتبه مباشرةً.
 *
 * وموضوعُ هذا الملفّ **البطاقةُ لا الاسم**: هل تُقرأ الإدارةُ المفردة
 * والقائمةُ قراءةً واحدة؟ فبقي السؤال وتبدّلت المركبة إلى `sectionId` —
 * حقلٌ يكتبه مديرُ الإدارة في إدارته وحدها، فيقيس ما كان يقيسه الاسم
 * بالضبط. ولو تُرك على الاسم لَصار يقيس البوابة الجديدة لا البطاقة.
 */
const rename = {
  sectionId: "s-new",
};
...
describe("بطاقةٌ بالمفرد وحده", () => {
  ...
  // (والمركبةُ اليوم `sectionId` لا الاسم — راجع تعليق `rename` أعلاه.)
  test("مديرُ الإدارة يعدّل مشروع إدارته", async () => {
    await seed();
