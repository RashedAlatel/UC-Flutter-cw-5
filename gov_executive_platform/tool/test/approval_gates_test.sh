...
# ونظيرُ `reject` **داخل أوّل كتلةٍ بعينها** لا في الملفّ كلِّه.
#
# ــ ولماذا لا يكفي `reject` هنا ــ
#
# `grep` لا يقرأ `\n` في نمطه. فقاعدةٌ تُكتب بسطرين — «لا يجتمع هذا مع
# ذاك» — لا يوجد نمطُها بحال، فتنجح **فراغاً**: تقول «لم يوجد ما يجب ألّا
# يوجد» وهي لم تنظر شيئاً. وكانت قاعدتان في هذا الملفّ كذلك، كشفتهما طفرةٌ
# نجت: أدرجتُ النوعَ في فرع المراحل فبقي الحارسُ راضياً.
#
# ــ وbawk لا sed ــ
#
# ومدى `sed -n '/من/,/إلى/p'` **يُفتح ثانيةً** كلَّما تكرّرت «من» بعد
# «إلى»، فيبتلع كتلاً أُخرى ويقول ما ليس فيها. وقد وقع ذلك في أوّل صياغةٍ
# لهذه القاعدة: فرعُ البتّ وفرعُ التطبيق كلاهما فيه `case "projectEdit"`.
# فـ`awk` يقف عند أوّل `default:` ولا يعود.
#
# وتُلتقط المخرجاتُ في متغيّر ثم تُفحص — لا `sed | grep -q`: قطعُ الأنبوب
# عند أوّل مطابقة يجعل نتيجةَ الفحص تتذبذب بين تشغيلةٍ وأخرى، وحارسٌ
# متذبذب أسوأُ من لا حارس.
reject_between() {
  local file="$1" name="$2" from="$3" to="$4" pattern="$5"
  local block
  block="$(awk -v f="$from" -v t="$to" '
    index($0, f) { seen = 1 }
    seen { print }
    seen && index($0, t) { exit }
  ' "$file")"
  if printf '%s\n' "$block" | grep -q "$pattern"; then
    echo "  ✗ $name"
    echo "      وُجد في كتلة «${from}» بـ${file} ما يجب ألّا يوجد: ${pattern}"
    FAIL=$((FAIL + 1))
  else
    echo "  ✔ $name"
    PASS=$((PASS + 1))
...
# ــــ نقلُ **موظّف** بين الإدارات: بوابةٌ من الصنف نفسه ــــ
#
# يرفعه مديرُ الإدارة ولا يبتّ فيه: النقلُ يغيّر بطاقةَ دخول الموظّف وما
# يراه من مشاريع الوزارة كلِّها. ولو أُدرج في فرع المراحل مع `projectEdit`
# و`deadlineChange` لَاعتمده مديرُ الإدارة عند مرحلته — وهي مرحلةٌ لا
# ينبغي أن تكون له أصلاً في هذا النوع.
#
# **وهو غيرُ `departmentTransfer`**: تلك تنقل مشروعاً وهذه تنقل إنساناً،
# واسماهما متقاربان — وقد اصطدما فعلاً عند كتابة هذه الدفعة.
echo "ونقلُ الموظّف بين الإدارات لمسؤول النظام وحده:"
want "النوعُ في فرع مسؤول النظام لا في فرع المراحل" 'و userTransfer: مسؤول النظام وحده'
reject_between "$SRC" "ولا يُدرج مع الأنواع ذات المرحلتين في بوابة البتّ" \
  'case "projectEdit":' 'default:' 'case "userTransfer":'
want_in "lib/data/app_store.dart" "والعميلُ مرآتُه: يُردّ مع الطلبات المحصورة" \
  "case ApprovalType.userTransfer:"
reject_between "lib/data/app_store.dart" "ولا يُدرج في مجموعة الأنواع ذات المرحلتين" \
  'kTwoStageApprovals = {' '};' 'ApprovalType.userTransfer'
echo ""

# ــ واعتمادُ التغيير التقنيّ: مفتاحُه في ذراعه وحدَها ــ
#
# `cab` تفتح بوّابةً لم تكن مفتوحة. وخطرُها ليس فيما تفتحه — ذاك مقصود —
# بل في أن تتسرّب إلى ذراعٍ أخرى بتعديلٍ لاحقٍ يبدو بريئاً، فتصير مفتاحاً
# يعتمد تسجيلَ الأعضاء أو تعديلَ المواعيد.
#
# فيُقاس موضعان: أنّ لها ذراعَها، وأنّ الفرعَ الافتراضيّ (حيث تقع البوّابات
# الثلاث) ما زال **مسؤولَ النظام وحدَه**.
echo "واعتمادُ التغيير التقنيّ في ذراعه، والبوّاباتُ في الافتراضيّ:"
want "الذراعُ موجودةٌ ومفتاحُها cab" 'allowed = isAdmin || perms?.cab === true;'
want "والفرعُ الافتراضيُّ مسؤولُ النظام وحدَه" '    default:'
want "ولا يزيد عليه شيء" '      allowed = isAdmin;'

# ــ ولا يظهر `cab` في شيفرةٍ ثالثة ــ
#
# موضعان لا ثالثَ لهما: تعريفُه في قائمة المفاتيح، وذراعُه في التخويل.
# وثالثٌ يعني أنّه تسرّب إلى بوّابةٍ أخرى.
#
# **والتعليقاتُ تُسقَط**: أوّلُ كتابةٍ لهذا الفحص عدّتها فسقط على نفسه —
# والحارسُ الذي يعدّ شرحَه لا يعدّ ما يحرسه.
CAB_CODE="$(grep -n 'cab' functions/src/index.ts \
  | grep -v '^[0-9]*:[[:space:]]*//' | grep -v '^[0-9]*:[[:space:]]*\*')"
CAB_USES="$(printf '%s\n' "${CAB_CODE}" | grep -c .)"
echo "ولا يتسرّب cab إلى شيفرةٍ ثالثة:"
if [ "${CAB_USES}" -eq 2 ]; then
  echo "  ✔ موضعان: التعريفُ وذراعُ التخويل"
  PASS=$((PASS + 1))
else
  echo "  ✗ ${CAB_USES} مواضعَ في الشيفرة — أيُّها ذراعٌ جديدة؟"
  printf '%s\n' "${CAB_CODE}" | sed 's/^/      /'
  FAIL=$((FAIL + 1))
fi
echo ""
...
# ورُفع إلى ١٣ عن قصد: «تصحيحُ نسبة إنجاز مشروع» فعلٌ جديد يمسّ مشروعاً،
# وقد خُتم بهدفه — فالرقمُ يتبعه ويُقرأ رفعُه في المراجعة.
STAMPED="$(grep -c "targetType: 'project'" lib/data/app_store.dart)"
if [ "$STAMPED" = "13" ]; then
  echo "  ✔ ثلاثةَ عشرَ فعلاً مختوماً في العميل"
  PASS=$((PASS + 1))
else
  echo "  ✗ ثلاثةَ عشرَ فعلاً مختوماً في العميل"
  echo "      عُدَّ $STAMPED لا ١٣ — أحُذف ختمٌ من فعلٍ يمسّ مشروعاً، أم أُضيف فعلٌ ولم يُرفع الرقم؟"
  FAIL=$((FAIL + 1))
fi
