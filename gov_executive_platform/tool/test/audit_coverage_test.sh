...
recordMyStatus
"
# وملحوظةٌ على `recordMyStatus`: **قرارٌ لا إغفال**. السجلُّ اليوميُّ نفسُه
# هو الأثر — يحمل صاحبَه ويومَه ووقتَ كتابته — ولا شيء يضيفه سطرٌ ثانٍ في
# سجلّ التدقيق. ومئتا موظّفٍ في مئتين وخمسين يومَ عملٍ يعني **خمسين ألفَ
# سطرٍ في السنة** يغرق فيها ما يُفتح السجلُّ لأجله: من غيّر ما ليس له.
#
# أمّا **التصحيح** فيُسجَّل: هو تغييرٌ على سجلٍّ محفوظٍ يحمل اسمَ من غيّره
# وما كان قبله — وذلك في `correctStatus` على الخادم أعلاه.
...
#
# ثم إلى ٣١ لثلاثٍ من حالات الموظفين اليومية.
#
# `correctStatus` و`setSectionHead`: كلتاهما تكتب سطرَها على
# الخادم. و`correctStatus` تحمل فيه **ما كان وما صار** حقلاً حقلاً، إذ
# «صُحِّح سجلُّ فلان» لا يجيب عن السؤال الذي يُفتح السجلُّ لأجله. و
# `setSectionHead` تسجّل من عُيِّن ومن عُزل، فرئاسةُ القسم تفتح نافذةً على
# بياناتٍ شخصيّة ولا تُمنح بلا أثر.
#
# و`recordMyStatus` في القائمة الثانية لا الأولى — راجع ملحوظتَها هناك.
ALLOWED_COUNT="$(printf '%s\n' "$ALLOWED" | grep -c .)"
if [ "$ALLOWED_COUNT" -le 31 ]; then
  report "وقائمة الاستثناء لم تتضخّم ($ALLOWED_COUNT من ٣١)" 1
else
